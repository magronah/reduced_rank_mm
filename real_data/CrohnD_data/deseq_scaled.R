...
for(i in 1:ncol(ddd)){
  ...
  pars[names(pars) == "betadisp"] <- log(1/0.3*result$dispersion[[i]])
...
saveRDS(deseq_aicc, file=paste0(file_path,"deseq_aicc_scaled.rds"))
saveRDS(res, file=paste0(file_path,"deseq_res_scaled.rds"))
