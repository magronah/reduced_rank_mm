...
for(i in 1:ntaxa) {
  ...
  betazi    =  as.numeric(unlist(fit$zi.fit[1]$coefficients))
  
  betadisp  =   as.numeric(unlist(fit$theta))
  sim_pars  =   lst(beta, betadisp, betazi, theta = log(theta_val), b)
...
saveRDS(sim_dd, file=paste0(path,"sim_data/zinbmm_otu_meta_list_withzi_taxa.rds"))
##############################################################################
