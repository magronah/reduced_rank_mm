...
for(i in 1:length(mod)){
  ...
  ####################################################################  
  nam  =  names(mod)[[i]]
  saveRDS(res_dd1, file = paste0(path, nam,"_sim_count_list_nozi.rds"))
  saveRDS(res_dd2, file = paste0(path, nam, "_otu_meta_list_nozi.rds"))
  ####################################################################  
  ...
  ####################################################################  
  saveRDS(ztaxa_res_dd1, file = paste0(path, nam,"_sim_count_list_withzi_taxa.rds"))
  saveRDS(ztaxa_res_dd2, file = paste0(path, nam,"_otu_meta_list_withzi_taxa.rds"))
  
  mean_count  =   colMeans(do.call(rbind,lapply(ztaxa_res_dd2, function(x) (x$countdata))))
  saveRDS(mean_count, file = paste0(path, nam,"_mean_count.rds"))
}
