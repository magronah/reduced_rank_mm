...
############################################################
path = paste0("~/scratch/dataset/RR/coverage/",nsubj,"_",ntaxa,"/nbmm2")
path
...
res = foreach(i = files, .combine ="cbind") %do% {
  mod  =   readRDS(i)
  lapply(mod,function(x) {summary(x)$coefficients$cond[, "Pr(>|z|)"][["grouptreat"]]})
}
...
saveRDS(dd, file = paste0("cov2/",nsubj,"_",ntaxa,"/pval_nbmm.rds"))
