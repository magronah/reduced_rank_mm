...
g1 = ggplot(dd, aes(x = param_name, y = average_estimate, group = model, 
                    color= factor(model, levels = c("rr", "zinbmm","true_param")))) +
  geom_point() +
  geom_point(aes(x = param_name, y = true_param,color="true_param")) +
  geom_line() +
  scale_color_manual(values = c("rr" = "red", "zinbmm" = "orange", "true_param" = "blue")) +
  labs(color = "model") +   
  xlab("taxa") +
...
###################################################################
##### Compare their MSE
rr_zi_error    =   error_cal(rr_zi_est, true_param, model = "rr")
zinbmm_error   =   error_cal(zinbmm_est, true_param, model = "zinbmm")

full_summary_dd_all         =    rbind(rr_zi_error$full_summary_dd,
                                       zinbmm_error$full_summary_dd)

single_summary_dd_all       =    rbind(rr_zi_error$single_sum_dd,
                                       zinbmm_error$single_sum_dd)
###################################################################
g2= ggplot(full_summary_dd_all, aes(x =  param_name, y = mse, 
                                    group = model, color = model)) +
  geom_point() +
  geom_line() +
  scale_color_manual(values = c("rr" = "red", "zinbmm" = "orange")) +
  xlab("taxa") +
...
mean(cov$coverage)
cov2 = coverage_cal(zinbmm_para_conf)
mean(cov2$coverage)
###################################################################
