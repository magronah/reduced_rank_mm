...
for(i in 1:length(zinbmm_mod$fit)){
  i=1  
  n  =  deseq_mod$result$param_name
  y  =  dd[,n[i]]
  
  ## fit model in glmmTMB
  modA   =   glmmTMB(y ~ group,
                     data = met_dd, 
                     family = nbinom2())
  
  
  ## set-up model in glmmTMB
  modB1  =   glmmTMB(y ~ group,
                     data = met_dd, 
                     family = nbinom2(),
                     doFit = FALSE)
  ...
  par_dd <- log(2)*coef(deseq_mod$object)
  dispersions <- dispersions(deseq_mod$object)
  f   =  par_dd[i,]
  
  pars[names(pars) == "beta"] <- f
  pars[names(pars) == "betadisp"] <- log(dispersions[i]/mean(dispersions))
  
  
  ## drop RE parameters
  pars <- pars[names(pars) != "b"]
  
  par2 <- with(modA$obj$env, last.par.best)
